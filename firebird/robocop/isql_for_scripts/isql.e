...
static void	compute_script_position (TEXT *, ULONG, BOOLEAN, TEXT *);
...
static SSHORT	frontend (TEXT *, ULONG *, ULONG *);
static SSHORT	get_statement (TEXT **, USHORT *, TEXT*, ULONG *, ULONG *);
...
static SSHORT	input (TEXT *, ULONG *, ULONG *);
...
static int	process_statement (TEXT *, XSQLDA **, ULONG *);
...
#else
static TEXT	*CurrentInputFileName = 0;
#endif 
 ...
 *	Shortcut for the next function, using global line = zero.
 **************************************/
	ISQL_errmsg2 (status, 0);
}
 ...
 *	Report error conditions
 *	Simulate isc_print_status exactly, to control ib_stderr
 *	The gcount parameter gives the global line in a script.
 **************************************/
TEXT	errbuf [MSG_LENGTH];
TEXT	script_msg [BUFFER_LENGTH80] = "";
STATUS	*vec;
...
if (Quiet)
	...
	STDERROUT (errbuf, 1);
	if (!*script_msg)
		compute_script_position (errbuf, gcount, FALSE, script_msg);
#else
    	buffer_error_msg (errbuf, WISQL_DETAILEDMSG);
    	buffer_error_msg (NEWLINE, WISQL_DETAILEDMSG);
#endif
    ...
    */
	...
	if (*script_msg)
		{
		STDERROUT (script_msg, 1);
		}
	else 
		{
		compute_script_position ("", gcount, TRUE, script_msg);
		STDERROUT (script_msg, 1);
		}
#endif
    }
...
ret  = process_statement (string, sqldap, 0);
if (ret == SKIP)
    ret = 0;
...
static void compute_script_position (
	TEXT	*input_msg,
	ULONG	gcount,
	BOOLEAN	force,
	TEXT	*errbuf)
...
/**************************************
 *
 *	c o m p u t e _ s c r i p t _ p o s i t i o n
 *
 **************************************
 *
 * Functional description
 * 	Given an error message translated by isc_interprete(),
 *	look for a pattern of (line, column) information and
 *	if present, convert it to a script-wide line count.
 *	This function assumes errbuf is at least 80 bytes long
 *	and that errbuf comes initialized to a blank string.
 *
 **************************************/
if (!Input_file)
	return;
...
if (force)
	{
	sprintf (errbuf, "-Script after line %ld\n", gcount);
	TRANSLATE_CP;
	}
else
	{
	/* There are two forms:
	Token unknown - line <l>, char <c>
	Column unknown \n <col_name> \n At line <l>, column <c>.
	*/
	ULONG ln = 0, cl = 0;
	SSHORT iter;
	SCHAR *tokstr [4] = {"- line ", ", char ", "At line ", ", column "};
	for (iter = 0; iter < 2; ++iter)
		{
		SCHAR *s0 = tokstr [iter * 2];
		SCHAR *s1 = tokstr [iter * 2 + 1];
		SCHAR *lnpos = strstr (input_msg, s0), *clpos = 0;
		if (lnpos)
			clpos = strstr (input_msg, s1);
		if (lnpos && clpos)
			{
			ln = atoi (lnpos + strlen (s0));
			cl = atoi (clpos + strlen (s1));
			sprintf (errbuf, "-Script line %ld, column %ld\n", ln + gcount, cl);
			TRANSLATE_CP;
			break;
			}
		}
...
/* Now, try to append the file name, without going past the buffer' size. */
if (CurrentInputFileName && errbuf[0])
	{
	TEXT *p = CurrentInputFileName;
	TEXT *prep = " in ";
	SSHORT blen = strlen (errbuf) - 1;
	SSHORT fnlen = strlen (p);
	SSHORT prlen = strlen (prep);
	SSHORT reslen = BUFFER_LENGTH80 - blen - prlen;
	if (fnlen >= reslen)
		{
		TEXT *spos = strrchr (p, '\\');
		if (!spos)
			spos = strrchr (p, '/');
		assert (!spos || spos < p + fnlen - 1);
		if (spos)
			{
			p = spos + 1;
			fnlen = strlen (p);
	...
	strcpy (errbuf + blen, prep); /* Overwrite the previous \n */
	strncpy (errbuf + blen + prlen, p, fnlen);
	strcpy (errbuf + blen + prlen + fnlen, "\n");
	}
}
...
while (!done)
    ...
    ret = get_statement (&statement, &bufsize, sql_prompt, &lcount, &gcount);
	...
	{   
	case CONT:
	    (void) process_statement (statement, sqldap, &gcount);
	    Continuation = FALSE;
	    break;
	...
	case EXIT:
		...
		if (CurrentInputFileName)
			{
			ISQL_FREE (CurrentInputFileName);
			CurrentInputFileName = 0;
			}
#endif
	    done = TRUE;
	    break;
...
/* perform scrolling within the currently active cursor */
	...
	(!strcmp (parms [0], "IN")))
	{
        /* Input_file = TRUE; Done conditionally in input(), only if success. */
        ret = input (lparms [1], lines, global_lines);
	}
 ...
 *		Input pointer to the prompt in interactive mode.
 *		Count of lines as seen by this function => \n
 *		Count of accumulated global lines.
 *
 **************************************/
TEXT	*p, *q;
...
while (!done)
	...
	case EOF:
		...
		if (lcount)
			*lcount = 0;
		if (global_lines)
			*global_lines = flist->global_lines + flist->lines;
#ifndef GUI_TOOLS
		/* Free the original iutput file name */
		if (CurrentInputFileName)
			{
			ISQL_FREE (CurrentInputFileName);
			CurrentInputFileName = flist->InputFileName;
			}
#endif
	...
	*/
	...
	ret = frontend (statement, lcount, global_lines);	
    }
...
if (fp = ib_fopen (file, "r"))
	...
	*lines = 0;
    ...
    if (global_lines)
    	{
	current->global_lines = *global_lines;
	*global_lines = 0;
    	}
    else
    	current->global_lines = 0;
...
#ifndef GUI_TOOLS
	/* Save original input file for ISQL */
	current->InputFileName = CurrentInputFileName;
	CurrentInputFileName = NULL;
	CurrentInputFileName = (TEXT *) ISQL_ALLOC (strlen(file) + 1);
	strcpy (CurrentInputFileName, file);
#ifdef DEBUG_GDS_ALLOC
	gds_alloc_flag_unfreed ((void *) CurrentInputFileName);
#endif
#endif
...
while (i < argc)
    ...
    */
	...
	if (c == 'X' || (c == 'E' && UPPER (s [2]) == 'X'))
	    ...
	    Can't pass lines and global_lines because we don' have a
	    call to do_isql() yet. It' irrelevant, since in non-inteactive
	    mode, finishing the main input file stops all processing. */

	    if ((s = argv [++i]) && (input (s, 0, 0) != FAIL))
		;
	    else
		ret = ERR;
	    Interactive = FALSE;
...
if (isc_dsql_prepare (isc_status, &prepare_trans, &Stmt, 0, string, 
    ...
    ISQL_errmsg2 (isc_status, *gcount);
    return ERR;
    }

/* check for warnings */
if (isc_status[2] == isc_arg_warning)
    ISQL_warning2 (isc_status, *gcount);
