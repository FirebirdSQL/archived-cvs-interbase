...
flag = SYNC | O_RDWR | O_CREAT | (overwrite ? O_TRUNC : O_EXCL) | O_BINARY;
#elif defined(SUPPORT_RAW_DEVICES)
flag = O_RDWR |
...
if (file->fil_desc == -1)
    unix_error ("PIO_header", file, isc_io_read_err, NULL_PTR);
...
for (i = 0; i < IO_RETRY; i++)
    ...
    if (dbb->dbb_encrypt_key)
...
#ifdef PREAD_PWRITE
        if ((bytes = pread (file->fil_desc, spare_buffer, length, 0)) == -1)
	    {
#else
	    ...
	    if (SYSCALL_INTERRUPTED(errno))
                continue;
            unix_error ("read", file, isc_io_read_err, NULL_PTR);
            }
...
if (i == IO_RETRY)
    ...
    ib_fflush (ib_stderr);
#endif
    	unix_error ("read_retry", file, isc_io_read_err, NULL_PTR);
	}
    }
#ifndef PREAD_PWRITE
THD_MUTEX_UNLOCK (file->fil_mutex);
...
#ifdef SUPPORT_RAW_DEVICES
/* At this point the file has successfully been opened in either RW or RO mode.
 * Check if it is a special file (i.e. raw block device) and if a valid
 * database is on it. If not, return an error.
 */
...
if ((status = status_vector) != (STATUS *)0)
    {
    *status++ = isc_arg_gds;
    *status++ = isc_io_error;
 ...
 */
if ((hp->hdr_ods_version < ODS_VERSION8) || (hp->hdr_ods_version > ODS_VERSION))
#else
