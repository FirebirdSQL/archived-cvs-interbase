...
{ dtype_null,			/* dtype_null through dtype_varying  */
...
/* Can we have a lightweight, binary comparison? */
    ...
    if (!bin_cmp)
	{
		...
		{
			obj1 = INTL_obj_lookup (tdbb, type_texttype, ttype1, err, NULL);
			obj2 = INTL_obj_lookup (tdbb, type_texttype, ttype2, err, NULL);
			ttype1 = obj1->texttype_type;
			ttype2 = obj2->texttype_type;
		}
    ...
    if (!bin_cmp && (blob1->blb_length > BUFFER_LARGE || blob2->blb_length > BUFFER_LARGE))
    {
		...
		assert (obj1);
		assert (obj2);
		if (obj1->texttype_bytes_per_char != 1 || obj2->texttype_bytes_per_char != 1)
			(*err)(gds__wish_list, 
			gds_arg_gds, gds__datnotsup, 0);
    }
...
#if defined(STACK_REDUCTION)
	/* do a block allocate */
    temp_str = (STR) ALLOCDV (type_str, ((SLONG)(sizeof (UCHAR) * (2 * BUFFER_LARGE))) );
...
#if defined(STACK_REDUCTION)
	/* do a block allocate */
    temp_str = (STR) ALLOCDV (type_str, ((SLONG)(sizeof (UCHAR) * (arg2->dsc_length))) );
...
assert (address != NULL);
assert (err != NULL);
assert ((((temp != NULL) && (length > 0)) || ((INTL_TTYPE(desc) <= dtype_any_text) && (INTL_TTYPE(desc) == to_interp)))  || (ptr != NULL));
...
if (desc->dsc_dtype == dtype_text)
    ...
    from_len = MIN (varying->vary_length, desc->dsc_length - sizeof (SSHORT));
    from_interp = INTL_TTYPE (desc);
    }
